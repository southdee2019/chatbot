...
const Panel = ({
  ...
  opacity = 100,
  children,
  contentStyle = "flex-col justify-center",
  ...props
}) => {
  const _clsName = {
  ...
  const Content = () => {
    let _bgfixed = "hero min-h-screen min-w-screen";
    // if (typeof window !== "undefined") { _bgfixed = (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(window.navigator.userAgent)) ? "hero min-h-screen min-w-screen" : "hero min-h-screen min-w-screen bg-fixed"; }
    return (
      <div
        className={_bgfixed}
